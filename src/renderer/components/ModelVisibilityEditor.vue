<template>
  <section class="form-section model-visibility" :aria-busy="loading">
    ...
    <p v-if="error || catalogWarning" role="alert">{{ error || catalogWarning }}</p>
    ...
    <p v-else-if="!loading && !error && !catalogWarning" class="empty-catalog">{{ t('runtimeModels.empty') }}</p>
...
const props = defineProps<{ visibility?: RuntimeModelVisibility; catalogWarning?: string; catalogKey: string; loadCatalog: (refresh: boolean) => Promise<RuntimeCatalog>; saveVisibility: (visibility: RuntimeModelVisibility) => Promise<void> }>()
...
const reset = () => { visibility.value = JSON.parse(JSON.stringify(props.visibility || {})); baseline.value = JSON.stringify(visibility.value); saved.value = false }
watch(() => JSON.stringify(props.visibility || {}), value => {
  if (value !== baseline.value) reset()
}, { immediate: true })
...
const load = async (refresh = false) => {
  ...
  try {
    const result = await props.loadCatalog(refresh)
...
const save = async () => {
  ...
  try {
    await props.saveVisibility(JSON.parse(JSON.stringify(visibility.value)))
    baseline.value = JSON.stringify(visibility.value); saved.value = true
